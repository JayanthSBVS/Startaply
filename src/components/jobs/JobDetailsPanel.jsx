    ...
    <div className={`fixed inset-0 z-[100] flex justify-end transition-opacity duration-300 ${isVisible ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
      ...
      <div className={`relative w-full max-w-[500px] bg-white h-full flex flex-col shadow-2xl transition-transform duration-300 transform ${isVisible ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-start bg-slate-50 shrink-0">
          <div>
            <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight leading-tight pr-4">{job.title}</h2>
            <p className="text-sm font-bold text-emerald-600 mt-1">{job.company}</p>
          </div>
          <div className="flex gap-2 shrink-0">
            <button onClick={handleShare} className="p-2 bg-emerald-50 hover:bg-emerald-100 rounded-full text-emerald-600 transition-colors border border-emerald-100"><Share2 size={18} /></button>
            <button onClick={() => { setIsVisible(false); setTimeout(onClose, 300); }} className="p-2 hover:bg-slate-200 rounded-full text-slate-500 transition-colors"><X size={20} /></button>
          </div>
        </div>

        {/* BODY */}
        {showForm ? (
          <div className="p-6 space-y-6 overflow-y-auto flex-1 bg-white custom-scrollbar">
            <button onClick={() => setShowForm(false)} className="text-slate-400 hover:text-slate-800 text-sm font-bold flex items-center gap-2 transition-colors mb-2">&larr; Back to Details</button>
            <div>
            ...
            <form id="applyForm" onSubmit={submitApplication} className="space-y-5">
              <div>
                ...
                <input required type="text" value={formData.name} onChange={e => { setFormData({ ...formData, name: e.target.value }); setErrors({ ...errors, name: null }); }} className={`w-full border rounded-xl px-4 py-3 outline-none transition-all font-medium ${errors.name ? 'border-rose-400 focus:ring-rose-500/20' : 'border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20'}`} />
              ...
              <div>
                ...
                <input required type="email" value={formData.email} onChange={e => { setFormData({ ...formData, email: e.target.value }); setErrors({ ...errors, email: null }); }} className={`w-full border rounded-xl px-4 py-3 outline-none transition-all font-medium ${errors.email ? 'border-rose-400 focus:ring-rose-500/20' : 'border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20'}`} />
              ...
              <div>
                ...
                <input type="text" placeholder="e.g. 9876543210" value={formData.phone} onChange={handlePhoneChange} className={`w-full border rounded-xl px-4 py-3 outline-none transition-all font-medium ${errors.phone ? 'border-rose-400 focus:ring-rose-500/20' : 'border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20'}`} />
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Resume (PDF/Doc)</label>
                <input type="file" accept=".pdf,.doc,.docx" onChange={handleFileUpload} className="w-full border border-slate-200 rounded-xl px-4 py-3 font-medium text-slate-600 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-bold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 cursor-pointer" />
              </div>
            </form>
          </div>
        ) : (
          <div className="p-6 space-y-8 overflow-y-auto flex-1 custom-scrollbar">
            ...
            {(job.processType === 'Voice Process' || job.processType === 'Non-Voice Process') && job.mapLocationUrl && (
              <div>
                <h3 className="text-sm font-bold uppercase tracking-wider text-emerald-600 mb-3 border-b border-slate-100 pb-2 flex items-center gap-2">
                  <MapPin size={16} /> Interview Walk-in Location
                </h3>
                <div className="w-full h-64 bg-slate-100 rounded-2xl overflow-hidden border border-slate-200 shadow-inner">
                  {job.mapLocationUrl.includes('<iframe') || job.mapLocationUrl.includes('embed') ? (
                    ...
                    <a href={job.mapLocationUrl} target="_blank" rel="noreferrer" className="w-full h-full flex flex-col items-center justify-center text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 transition-colors">
            ...
            {job.aboutCompany && (
              <div>
                <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400 mb-3 border-b border-slate-100 pb-2">About {job.company}</h3>
                <div className="text-slate-600 font-medium leading-relaxed text-[15px] whitespace-pre-wrap">{job.aboutCompany}</div>
              </div>
            )}
          </div>
        )}
        ...
        {/* FOOTER ACTION */}
        <div className="p-6 border-t border-slate-100 bg-white shrink-0 shadow-[0_-4px_15px_-3px_rgba(0,0,0,0.05)] z-20">
          {showForm ? (
            <button type="submit" form="applyForm" disabled={isSubmitting || applied} className={`w-full py-4 rounded-full font-bold text-lg flex justify-center items-center gap-2 transition-all ${applied ? 'bg-emerald-100 text-emerald-700' : 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-600/30'}`}>
            ...
            <button onClick={() => setShowForm(true)} className="w-full py-4 rounded-full font-bold text-lg flex justify-center items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white transition-all shadow-lg shadow-emerald-600/30">
