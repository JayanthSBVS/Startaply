...
const TrendingCompanies = () => {
  ...
  // Multiply companies to ensure seamless marquee scrolling even with few items
  ...
  const stream3 = repeatToCount(companies, 20);
  ...
  return (
    <section className="py-24 relative overflow-hidden section-dark transition-colors duration-500">
      ...
      {/* Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60vw] h-[400px] rounded-full opacity-20 pointer-events-none mix-blend-screen"
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10 mb-16 text-center">
        <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full bg-slate-800/50 border border-slate-700/50 shadow-sm backdrop-blur-sm">
          <Users size={14} className="text-emerald-400" />
          <span className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-300">The Startaply Network</span>
        </div>
        ...
        <p className="text-slate-400 font-medium text-lg max-w-xl mx-auto mb-8">
          Join the exclusive network of top enterprises and startups recruiting directly through our platform.
        </p>
        <Link to="/companies" className="inline-flex items-center gap-2 px-8 py-3.5 bg-white text-slate-900 rounded-full font-bold text-sm hover:bg-emerald-400 hover:text-slate-900 transition-all shadow-[0_0_30px_rgba(255,255,255,0.1)] hover:shadow-[0_0_40px_rgba(16,185,129,0.3)]">
          Explore Directory <ExternalLink size={16} />
        </Link>
      </div>
      ...
      <div className="relative z-10 flex flex-col gap-5 pb-10">
        ...
        <div className="overflow-hidden">
          <div className="companies-track">
            {stream1.map((company, i) => <CompanyCard key={`${company.id}-1-${i}`} company={company} />)}
          </div>
        </div>
        ...
        <div className="overflow-hidden">
          <div className="companies-track-reverse" style={{ transform: 'translateX(-50%)' }}>
            {stream2.map((company, i) => <CompanyCard key={`${company.id}-2-${i}`} company={company} />)}
          </div>
        </div>
      </div>
    </section>
