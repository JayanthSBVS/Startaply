    ...
    >
      <div className="relative mb-8">
        ...
        <div className="relative w-28 h-28 bg-slate-50 dark:bg-slate-950 rounded-full flex items-center justify-center border border-slate-100 dark:border-slate-800 shadow-inner overflow-hidden">
          ...
          >
            <SearchX size={48} className="text-slate-300 dark:text-slate-700" />
          </motion.div>
      ...
      <h3 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white mb-4 tracking-tight">{title}</h3>
      <p className="text-slate-500 dark:text-slate-400 max-w-sm mb-10 font-medium leading-relaxed md:text-lg">
        {message}
      </p>
      ...
      {onReset && (
        <button
          onClick={onReset}
          className="group inline-flex items-center gap-3 bg-slate-950 dark:bg-emerald-600 hover:bg-emerald-600 dark:hover:bg-emerald-500 text-white font-black py-4 px-10 rounded-full transition-all shadow-xl shadow-slate-950/20 dark:shadow-emerald-600/30 active:scale-95"
        >
